...
public:
  ...
  void SetBufferLocation(DataLocation location) override
  {
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OmTensor] `SetBufferLocation` Got invalid location: UNKOWN !");
  }
  ...
  void ToLocation(DataLocation location) override
  {
    assert(current_location_ != DataLocation::UNKOWN);
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OmTensor] `ToLocation` Got invalid location: UNKOWN !");
  }
  ...
  {
    CHECK_STATE_THROW(tensor != nullptr, "[OmTensor] `ZeroCopy` Got invalid tensor: nullptr !");
    auto location = tensor->GetBufferLocation();
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OmTensor] `ZeroCopy` Got invalid tensor location: UNKOWN !");
    ...
    CHECK_STATE_THROW(raw_ptr != nullptr,
                      "[OmTensor] `ZeroCopy` Got invalid tensor raw_ptr: nullptr !");
  ...
  {
    CHECK_STATE_THROW(tensor != nullptr, "[OmTensor] `DeepCopy` Got invalid tensor: nullptr !");
    auto location = tensor->GetBufferLocation();
    CHECK_STATE_THROW(location != DataLocation::UNKOWN,
                      "[OmTensor] `DeepCopy` Got invalid tensor location: UNKOWN !");
    if (location == DataLocation::DEVICE)
    {
      tensor->ToLocation(DataLocation::HOST);
    }
    auto raw_ptr = tensor->RawPtr();
    CHECK_STATE_THROW(raw_ptr != nullptr,
                      "[OmTensor] `DeepCopy` Got invalid tensor raw_ptr: nullptr !");
  ...
  void SetShape(const std::vector<size_t> &shape) override
  {
    ...
    CHECK_STATE_THROW(this_shape_byte_size <= GetBufferMaxByteSize(),
                      "[OmTensor] `SetShape` Got invalid shape: exceeds max byte size !");
